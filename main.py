...
def _acquire_single_instance():
    ...
    try:
        ...
        # handle 存到全域，避免被 GC 回收導致 mutex 釋放
        global _instance_mutex
        _instance_mutex = ctypes.windll.kernel32.CreateMutexW(
            None, False, 'Global\\VoiceTyperSingleInstance')
...
def main():
    ...
    log.info(f'啟動檢查：.env 存在={ENV_FILE.exists()}, 有任何 key={has_keys}')
    need_onboarding = not has_keys

    if need_onboarding:
        log.info('首次啟動 / .env 為空 → 顯示 Onboarding')
        wizard = OnboardingWizard(root, env_mgr, config_mgr)
        root.wait_window(wizard)
        load_dotenv(ENV_FILE, override=True)
        # 如果使用者直接 X 掉 wizard 且 .env 仍空，退出
        if not _has_any_existing_key():
            log.info('使用者跳過設定，退出')
            sys.exit(0)
