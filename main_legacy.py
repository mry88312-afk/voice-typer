...
from storage.env_manager import EnvManager
from storage.history_manager import HistoryManager
from storage.usage_manager import UsageManager
from storage.profile_manager import ProfileManager
from storage.learned_words_manager import LearnedWordsManager
...
def _resolve_base_dir() -> Path:
    """設定/資料的持久存放目錄。
    - 從 source 跑 (python main.py)：用程式碼所在目錄 (現有行為)
    - 從 .exe 跑 (PyInstaller frozen)：用 %APPDATA%\\VoiceTyper
      避免 PyInstaller onefile 把資料寫到 temp dir 重啟就消失
    """
    if getattr(sys, 'frozen', False):
        appdata = os.environ.get('APPDATA')
        if appdata:
            base = Path(appdata) / 'VoiceTyper'
        else:
            base = Path.home() / '.voice-typer'
...
# .exe 的程式本體位置 (用來找 .env.example / 預設 config 作初始化)
...
HALLUCINATIONS = [
    # YouTube 字幕殘留
    '字幕由amara.org社群提供', 'amaraorg', '字幕志愿者', 'mariapardiralscat',
    'subtitlesbysub', 'subtitleby', 'transcribedby',
    # 訂閱/觀看常見幻覺
    '请订阅', '請訂閱', '我要訂閱', '訂閱頻道', '订阅频道',
    '感謝您的觀看', '感谢您的观看', '感謝觀看', '感谢观看',
    '謝謝大家', '谢谢大家', '謝謝收看', '謝謝觀看',
    '小鈴鐺', '小铃铛', '按讚分享', '点赞分享',
    # 動漫/虛擬偶像殘留 (Whisper 訓練資料常見)
    '初音', '初音ミク', 'hatsunemiku', 'kizunaai',
    # 英日韓常見幻覺
    'thanksforwatching', 'thankyouforwatching',
    'subscribetomychannel', 'pleasesubscribe',
    'mbc뉴스', 'sbs뉴스', 'jtbc뉴스',
...
def is_hallucination(text: str) -> bool:
    if not text:
        return False
    normalized = ''.join(c for c in text.lower() if c.isalnum() or '一' <= c <= '鿿')
    if not normalized:
        return True
    # 黑名單 match
    if any(h in normalized for h in HALLUCINATIONS):
        return True
    # 短文字 (<= 8 字) 且 0 個中文字 → 可能是雜訊幻覺
    if len(normalized) <= 8:
...
class VoiceTyperApp:
    ...
    def _rebuild_transcriber(self):
        ...
        try:
            ...
            if secondary:
                from fallback_transcriber import FallbackTranscriber
...
# ---------- main ----------
def _ensure_user_data_initialized():
    """第一次啟動 .exe 時，把 bundle 中的預設檔案 copy 到 %APPDATA%"""
    if not getattr(sys, 'frozen', False):
        return  # source 模式不需要

    import shutil
    # 預設 config.json (從範本 copy；範本不含任何個人資料)
    if not CONFIG_FILE.exists():
        src = BUNDLE_DIR / 'config.example.json'
        if not src.exists():
            src = BUNDLE_DIR / 'config.json'  # 向後相容舊 build
        if src.exists():
            shutil.copy(src, CONFIG_FILE)
            log.info(f"初始化 config.json: {CONFIG_FILE}")
    # .env.example (使用者參考用)
    example_path = BASE_DIR / '.env.example'
    if not example_path.exists():
        src = BUNDLE_DIR / '.env.example'
        if src.exists():
            shutil.copy(src, example_path)
    # recordings 目錄
    RECORDINGS_DIR.mkdir(parents=True, exist_ok=True)


def _acquire_single_instance():
    """Windows mutex 確保只跑一個實例。回傳 False = 已有實例在跑。

    重點：必須用 WinDLL(use_last_error=True) + ctypes.get_last_error()，
    不能用 windll.kernel32.GetLastError()。後者在 ctypes 內部呼叫之間
    last error 會被覆蓋，導致永遠拿不到 ERROR_ALREADY_EXISTS (經典陷阱)。
    """
    try:
        import ctypes
        ...
        kernel32.CreateMutexW.argtypes = [wintypes.LPCVOID, wintypes.BOOL,
                                          wintypes.LPCWSTR]
        # 用 session-local 名稱 (不加 Global\，避免權限問題；每個登入工作階段一個鎖)
        global _instance_mutex
        _instance_mutex = kernel32.CreateMutexW(None, False,
                                                'VoiceTyperSingleInstance')
        last_error = ctypes.get_last_error()
        if _instance_mutex == 0:
            # CreateMutex 本身失敗 → 放行，不要因此開不了
            return True
        return last_error != ERROR_ALREADY_EXISTS
    except Exception:
        return True  # mutex 失敗就放行，不要因此開不了


def main():
    if not _acquire_single_instance():
        # 已有實例在跑 → 直接安靜退出。
        # 不彈阻塞式對話框 (會留殭屍 process + 容易被誤認成設定頁)。
        # app 已在托盤待命，使用者重複點捷徑不會有任何干擾。
    ...
    usage_mgr = UsageManager(USAGE_FILE)
    profile_mgr = ProfileManager(config_mgr)
    learned_mgr = LearnedWordsManager(LEARNED_WORDS_FILE)

    init_app_theme()

    # 隱藏的 tk root (所有 Toplevel 的 parent)
    root = ctk.CTk()
    root.withdraw()
    root.title('Voice Typer')

    # 檢查是否需要 onboarding
    # 新邏輯：只要 .env 存在且有任何一把非空 *_API_KEY，就不跳 wizard
    # （以前嚴格驗 sk- prefix，遇到 sk-proj- 之外的格式或讀取閃失就誤觸 wizard）
    def _has_any_existing_key():
        if not ENV_FILE.exists():
            return False
        try:
            data = env_mgr.read()
        except Exception as e:
            log.warning(f'讀取 .env 失敗: {e}')
            return False
    ...
    has_keys = _has_any_existing_key()
    setup_done = bool(config_mgr.get('setup_completed', False))
    log.info(f'啟動檢查：.env 存在={ENV_FILE.exists()}, '
             f'有任何 key={has_keys}, 已完成設定={setup_done}')

    # 既有用戶豁免：只要曾經有過 key，永久記住「設定完成」，之後絕不再跳設定頁
    # （避免開機時磁碟忙碌 / 防毒鎖檔導致 .env 讀取閃失，誤判成首次啟動）
    if has_keys and not setup_done:
        config_mgr.set('setup_completed', True)
        config_mgr.save()
        setup_done = True

    # 只有「從未設定過」且「現在也沒 key」才顯示 onboarding
    need_onboarding = (not setup_done) and (not has_keys)

    if need_onboarding:
        log.info('首次啟動 → 顯示 Onboarding')
        try:
            wizard = OnboardingWizard(root, env_mgr, config_mgr)
            root.wait_window(wizard)
            load_dotenv(ENV_FILE, override=True)
            # 使用者有填 key → 記住設定完成
            if _has_any_existing_key():
                config_mgr.set('setup_completed', True)
                config_mgr.save()
            else:
                # 關鍵改變：就算沒填 key，也「不」退出。
                # app 照常進入托盤待命，使用者隨時能從托盤 → 設定 補 key。
                # 關一個視窗絕不該殺掉整個程式。
                log.info('使用者跳過設定，仍進入托盤待命模式')
        except Exception as e:
            # onboarding 出任何錯都不能害 app 開不起來
            log.error(f'Onboarding 顯示失敗，略過直接進入主程式: {e}')

    # 啟動主程式（無論有沒有 key，一律走到這）
    app = VoiceTyperApp(
        root, config_mgr, env_mgr, history_mgr, usage_mgr, profile_mgr, learned_mgr,
    )
    app.start()
